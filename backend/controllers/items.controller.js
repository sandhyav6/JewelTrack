...
exports.getById = asyncHandler(async (req, res) => {
  const sql = `
    SELECT ITEMID, ITEMNAME, CATEGORYID, CATEGORYNAME, MATERIAL, WEIGHT, PRICE,
           AVAILABLEQUANTITY, STOCKSTATUS, RECENTSUPPLIER
    FROM V_INVENTORY WHERE ITEMID = :id
  `;
...
exports.create = asyncHandler(async (req, res) => {
  ...
  res.status(201).json({ success: true, data: { id, name, categoryId, material, weight, price, stock } });
});

exports.update = asyncHandler(async (req, res) => {
  const { name, categoryId, material, weight, price, stock } = validateItem(req.body);
  const result = await query(
    `UPDATE ITEM SET ITEMNAME=:name, CATEGORYID=:categoryId, MATERIAL=:material,
            WEIGHT=:weight, PRICE=:price, AVAILABLEQUANTITY=:stock
     WHERE ITEMID=:id`,
  ...
  res.json({ success: true, data: { id: req.params.id, name, categoryId, material, weight, price, stock } });
});
